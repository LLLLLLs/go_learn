...
class NovelReaderPanelTest {
    @Test
    fun chapterSelectorButtonTracksCurrentChapterForRealBook() {
        val bookFile = File("book/流氓帝师.epub")
        assertTrue(bookFile.isFile, "test book not found: ${bookFile.absolutePath}")
    ...
    @Test
    fun chapterSelectorPopupTracksHighChapterForRealBook() {
        val bookFile = File("book/流氓帝师.epub")
        assertTrue(bookFile.isFile, "test book not found: ${bookFile.absolutePath}")
