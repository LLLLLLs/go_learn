...
func (S *SysLogHandle) scanBuffer() {
	...
	for S.deamon {
		...
		//当count小于batchSize才去扫描文件是为了让程序才空闲的时候再去做这些事情
		if count < S.batchSize {
			S.scanFile()
		}
		count = 0
...
func (S *SysLogHandle) emit(b []byte) {
	...
	case S.limit <- 1:
		...
		if err != nil {
			fmt.Fprintln(os.Stderr, "syslog send fial write file", err)
			S.writeFile(b)
			conn.conn.Close()
			return
		}
...
func (S *SysLogHandle) createConn() {
	conn, err := net.DialTimeout("tcp", S.raddr, time.Millisecond*S.timeout)
	if err != nil {
...
func Dial(network, raddr string, priority Priority) (*SysLogHandle, error) {
